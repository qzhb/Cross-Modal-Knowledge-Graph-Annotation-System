...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def editLabel(self, item=False):
        ...
        if not self.validateLabel(text):
            ...
            return
        if str(shape.label) != str(text):
            x1 = int(shape.points[0].x())
            y1 = int(shape.points[0].y())
            x2 = int(shape.points[1].x())
            y2 = int(shape.points[1].y())
            img = cv2.imread(self.filename)
            x3 = max(x1 - int((x2 - x1) / 2), 0)
            x4 = min(x2 + int((x2 - x1) / 2), img.shape[1])
            y3 = max(y1 - int((y2 - y1) / 2), 0)
            y4 = min(y2 + int((y2 - y1) / 2), img.shape[0])
            cropped = img[y3:y4,x3:x4]
            filename = str(self.filename)
            tmp = filename.split('/')
            moviename = tmp[len(tmp) - 4]
            db = os.getcwd() + '/annotation_results/Face_data/' + moviename + '_DB/'
            if not os.path.exists(db+str(text)+'.jpg'):
                pic_name = db + str(text) + '.jpg'
            else:
                i = 1
                pic_name = db+str(text)+'%'+str(i)+'.jpg'
                while(os.path.exists(pic_name)):
                    i += 1
                    pic_name = db + str(text) + '%' + str(i) + '.jpg'
            cv2.imwrite(pic_name,cropped)
            self.signal = True
        shape.label = text
        shape.flags = flags
        item.setText(text)
    ...
    def IOU(self,cx1,cy1,cx2,cy2,gx1,gy1,gx2,gy2):
        ...
        ratio = abs(S_rec1-S_rec2)/min(S_rec1,S_rec2)
        if iou>=0.5 and ratio<0.35:
            return True
        else:
            return False
    ...
    def loadFile(self, filename=None):
        ...
        # 当前图片是否存在json文件，若存在，则不重新检测
        tmp = filename.split('/')
        clipname = tmp[len(tmp) - 3]
        moviename = tmp[len(tmp) - 4]
        picname = tmp[len(tmp) - 1].split('.')[0]
        json_path = os.getcwd() + '/annotation_results/annotation_data/' + moviename + '/' + clipname + '/json/'
        json_file = json_path + picname + '.json'
        db_ori = os.getcwd() + '/data/' + moviename + '/' + moviename + '_DB/'
        db = os.getcwd() + '/annotation_results/Face_data/' + moviename + '_DB/'
        ...
        if not os.path.exists(json_file):
            ...
            if not os.path.exists(db):
                if not os.path.exists(os.getcwd()+'/annotation_results/Face_data/'):
                    os.mkdir(os.getcwd()+'/annotation_results/Face_data/')
                os.system('cp -r '+db_ori+' '+db)
            if self.signal == False:
                command = os.getcwd() + filename[1:] + ' ' + db + " 0 "
            if self.signal == True:
                command = os.getcwd() + filename[1:] + ' ' + db + " 1 "
                self.signal = False
            socket.send(command.encode())
            message = socket.recv()
            #STR = repr(message).split('END')[0]

            #while not os.path.exists(os.getcwd()+'/Face/info.txt'):
                #time.sleep(0.5)
            f = open(os.getcwd()+'/Face/info.txt','r')
            info = f.readlines()[0]
            STR = info.split('END')[0]
            f.close()

            # 生成相应json文件
            NUM_FACE = STR.count('FACE', 0, len(STR))
            WIDTH = STR.split(': ')[1].split('Height')[0]
            if NUM_FACE == 0:
                HEIGHT = STR.split(': ')[2].strip()
            else:
                HEIGHT = STR.split(': ')[2].split('FACE')[0]
            FACES = STR.split('FACE: ')[1:]

            # 修改txt文件，实现示例窗口的实时更新
            if NUM_FACE > 0:
                list_path1 = os.getcwd()+'/show_example/update_Y.txt'
                list_path2 = os.getcwd() + '/show_example/update_N.txt'
                if os.path.exists(list_path1):
                    list_path = list_path1
                else:
                    list_path = list_path2
                f = open(list_path,'w')
                k = 0
                for i in range(5):
                    if k == 5:
                    ...
                    for j in range(NUM_FACE):
                        if k == 5:
                            break
                        f.write(FACES[j].split('ID'+str(i+1)+': ')[1].split('ID'+str(i+2)+': ')[0].split('/')[-1].split('.jpg')[0] + '\n')
                        k += 1
                f.close()
                os.rename(list_path,list_path1)
            ...
            f = open(json_path + picname + '.json', 'w')
            ...
            data['fillColor'] = [255, 0, 0, 64]
            data['imageData'] = None
            # else:
            #     f2 = open(self.lastpath,'r')
            #     data = json.load(f2)
            #     f2.close()
            data['imagePath'] = os.getcwd() + filename[1:]
            data['imageHeight'] = int(HEIGHT)
            data['imageWidth'] = int(WIDTH)
            for i in range(NUM_FACE):
                x1 = FACES[i].split('EMOTION')[0].split(',')[0]
                y1 = FACES[i].split('EMOTION')[0].split(',')[1]
                x2 = FACES[i].split('EMOTION')[0].split(',')[2]
                y2 = FACES[i].split('EMOTION')[0].split(',')[3]
                # for jj in range(len(self.deletelist)):
                #     j = self.deletelist[jj]
                #     if abs(j[0] - int(x1)) + abs(j[1] - int(y1)) + abs(j[2] - int(x2)) + abs(j[3] - int(y2)) < abs(
                #             int(x2) - int(x1)) / 4:
                #     # if self.IOU(j[0],j[1],j[2],j[3],int(x1),int(y1),int(x2),int(y2)):
                #         self.deletelist[jj][0] = int(x1)
                #         self.deletelist[jj][1] = int(y1)
                #         self.deletelist[jj][2] = int(x2)
                #         self.deletelist[jj][3] = int(y2)
                #         break
                # else:
                shapes = {}
                shapes['label'] = \
                FACES[i].split('ID1: ')[1].split('ID2: ')[0].split('/')[-1].split('.jpg')[0].split('%')[0]
                shapes['line_color'] = None
                shapes['fill_color'] = None
                shapes['shape_type'] = 'rectangle'
                shapes['flags'] = {}
                shapes['points'] = [[int(x1), int(y1)], [int(x2), int(y2)]]
                data['shapes'].append(shapes)
                # for ii in range(len(data['shapes'])):
                #     x3 = data['shapes'][ii]['points'][0][0]
                #     y3 = data['shapes'][ii]['points'][0][1]
                #     x4 = data['shapes'][ii]['points'][1][0]
                #     y4 = data['shapes'][ii]['points'][1][1]
                #     # if abs(int(x3) - int(x1)) + abs(int(y3) - int(y1)) + abs(int(x4) - int(x2)) + abs(
                #     #         int(y4) - int(y2)) < abs(
                #     #         int(x2) - int(x1)) / 2:
                #     if self.IOU(int(x1),int(y1),int(x2),int(y2),int(x3),int(y3),int(x4),int(y4)):
                #         data['shapes'][ii]['points'][0][0] = int(x1)
                #         data['shapes'][ii]['points'][0][1] = int(y1)
                #         data['shapes'][ii]['points'][1][0] = int(x2)
                #         data['shapes'][ii]['points'][1][1] = int(y2)
                #         data['shapes'][ii]['label'] = shapes['label']
                #         break
                # else:
                #     data['shapes'].append(shapes)
                #     continue
            ...
            # self.lastpath = json_path + picname + '.json'
            f.close()
    ...
    def deleteSelectedShape(self):
        ...
        if True:
            ...
            x2 = int(shape.points[1].x())
            y2 = int(shape.points[1].y())
            # self.deletelist.append([x1,y1,x2,y2])
